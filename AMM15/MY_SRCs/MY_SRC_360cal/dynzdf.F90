MODULE dynzdf
   ...
   !!
   !!           ! This file has been adapted by AW to add improved bed friction estimation 
   !!           ! and later re-adpted by JR to output bed stress estimation 
   ...
   USE iom ! input/output library- JR add
...
!!AW local 3D variables for implicit bottom friction
      REAL(wp), DIMENSION(:,:,:), ALLOCATABLE ::   ztrdu_bfri, ztrdv_bfri
      REAL(wp), DIMENSION(:,:,:), ALLOCATABLE ::   zua, zva
      REAL(wp), DIMENSION(:,:),   ALLOCATABLE ::   zue, zve, z2du, z2dv
!!AW end
      !!---------------------------------------------------------------------
      !
      ...
      IF( kt == nit000 ) THEN       !* initialization
         IF(lwp) WRITE(numout,*)
         IF(lwp) WRITE(numout,*) 'dyn_zdf_imp : vertical momentum diffusion implicit operator BLADIBLA'
         IF(lwp) WRITE(numout,*) '~~~~~~~~~~~ '
         !
...
!!AW if implicit friction then we need to include the barotropic velocity
         IF( ln_drgimp .AND. ln_dynspg_ts ) THEN
            ...
            ALLOCATE( zua(jpi,jpj,jpk), zva(jpi,jpj,jpk) )
            ALLOCATE( zue(jpi,jpj), zve(jpi,jpj), z2du(jpi,jpj), z2dv(jpi,jpj) )
            ...
            END DO
            DO jk = 1, jpkm1
               zua(:,:,jk) = ( ua(:,:,jk) - zue(:,:) * r1_hu_a(:,:) + ua_b(:,:) ) * umask(:,:,jk)
               zva(:,:,jk) = ( va(:,:,jk) - zve(:,:) * r1_hv_a(:,:) + va_b(:,:) ) * vmask(:,:,jk)
            END DO
            ztrdu(:,:,:) = ( zua(:,:,:) - ub(:,:,:) ) / r2dt - ztrdu(:,:,:)
            ztrdv(:,:,:) = ( zva(:,:,:) - vb(:,:,:) ) / r2dt - ztrdv(:,:,:)
            CALL trd_dyn( ztrdu, ztrdv, jpdyn_zdf, kt )

            ! Now calcuate the friction explicitly using the adjusted after velocity
            ALLOCATE( ztrdu_bfri(jpi,jpj,1), ztrdv_bfri(jpi,jpj,1) )
            ztrdu_bfri(:,:,:) = 0._wp
            ztrdv_bfri(:,:,:) = 0._wp
            DO jj = 2, jpjm1       
               DO ji = fs_2, fs_jpim1   ! vector opt.
                  ...
                  ze3ua =  ( 1._wp - r_vvl ) * e3u_n(ji,jj,iku) + r_vvl * e3u_a(ji,jj,iku)
                  ze3va =  ( 1._wp - r_vvl ) * e3v_n(ji,jj,ikv) + r_vvl * e3v_a(ji,jj,ikv)
                  ztrdu_bfri(ji,jj,1) = 0.5*( rCdU_bot(ji+1,jj)+rCdU_bot(ji,jj) ) * zua(ji,jj,iku) / ze3ua
                  ztrdv_bfri(ji,jj,1) = 0.5*( rCdU_bot(ji,jj+1)+rCdU_bot(ji,jj) ) * zva(ji,jj,ikv) / ze3va
                  z2du(:,:) = rau0*ztrdu_bfri(:,:,1)*tmask(:,:,1)  ! JR bed stress zonal
                  z2dv(:,:) = rau0*ztrdv_bfri(:,:,1)*tmask(:,:,1)  ! JR bed stress meridional
               END DO
            END DO
            ! JR call in variables
            CALL trd_dyn( ztrdu_bfri, ztrdv_bfri, jpdyn_bfri, kt )
            !CALL iom_put( "ztrdu_bfri",ztrdu_bfri) !enregy from friction per unit mass (m2/s2) 
            !CALL iom_put( "ztrdv_bfri",ztrdv_bfri)
            CALL iom_put("utaubfri", z2du(:,:))
            CALL iom_put("vtaubfri", z2dv(:,:))
            ! JR end
            DEALLOCATE( zua, zva, zue, zve, ztrdu, ztrdv, ztrdu_bfri, ztrdv_bfri )
!!AW end
         ELSE
      ...
      ENDIF
      !
!!AW recompute the bottom friction using the after velocity trend for implicit bottom friction 
!!   this is only used to in the call to output the bottom friction momentum trend
!      IF( l_trddyn .AND. ln_drgimp ) THEN
!         ALLOCATE( ztrdu_bfri(jpi,jpj,jpk), ztrdv_bfri(jpi,jpj,jpk) )
!         ztrdu_bfri(:,:,:) = 0._wp
!         ztrdv_bfri(:,:,:) = 0._wp
!         DO jj = 2, jpjm1       
!            DO ji = fs_2, fs_jpim1   ! vector opt.
...
!               ze3ua =  ( 1._wp - r_vvl ) * e3u_n(ji,jj,iku) + r_vvl * e3u_a(ji,jj,iku)
!               ze3va =  ( 1._wp - r_vvl ) * e3v_n(ji,jj,ikv) + r_vvl * e3v_a(ji,jj,ikv)
!               ztrdu_bfri(ji,jj,iku) = 0.5*( rCdU_bot(ji+1,jj)+rCdU_bot(ji,jj) ) * ( ua(ji,jj,iku) + ua_b(ji,jj) ) / ze3ua
!               ztrdv_bfri(ji,jj,ikv) = 0.5*( rCdU_bot(ji,jj+1)+rCdU_bot(ji,jj) ) * ( va(ji,jj,ikv) + va_b(ji,jj) ) / ze3va
!            END DO
!         END DO
!         CALL trd_dyn( ztrdu_bfri, ztrdv_bfri, jpdyn_bfri, kt )
!         DEALLOCATE( ztrdu_bfri, ztrdv_bfri ) 
!      ENDIF
!!AW end     
